...
| Microsoft.Data.Sqlite | 9.0.10 (approved) | The accepted SQLite provider library for Odyssey VTT persistence, per `ADR-011` v1.1 §1, closing `ADR-011` v1.0 §12.1 on the `SP-02` spike's recommendation. Currently referenced only by the `SP-02` evidence-generation harness (`Tools/Spikes/SP-02-PersistenceReliability/`) — no production `.csproj` references it yet, since production persistence implementation is not in scope for `ODY-S01-005` | MIT | Approved for future production `Persistence` use per `ADR-011` v1.1; currently present only in `Tools/Spikes/SP-02-PersistenceReliability/`, not in `DotNet/Odyssey.Core.sln` | `ADR-011` v1.1 / `SP-02` report (`docs/tasks/completed/ODY-S01-005_SP-02_Persistence_Reliability_Report.md` §3) |
| SQLitePCLRaw.bundle_e_sqlite3 | >= 3.0.3 (approved, mandatory floor) | Transitive native SQLite bundle; `ADR-011` v1.1 §1 mandates this floor because `Microsoft.Data.Sqlite` 9.0.x otherwise pulls a `2.1.x` chain flagged by NuGet audit for a known high-severity vulnerability (`GHSA-2m69-gcr7-jv3q`) | MIT / Apache-2.0 | Same as above | `ADR-011` v1.1 / `SP-02` report |
