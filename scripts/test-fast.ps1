...
try {
    ...
    $resultsDir = Join-Path $repoRoot 'Logs/ODY-S00-006/dotnet'
    ...
    dotnet test .\DotNet\Odyssey.Core.sln --no-build --no-restore --logger 'trx;LogFilePrefix=ody-s00-006' --results-directory $resultsDir
    if ($LASTEXITCODE -ne 0) {
        exit $LASTEXITCODE
    }
    ...
    if ($resultFiles.Count -eq 0) {
        Write-Error "No .NET TRX result files were created under Logs/ODY-S00-006/dotnet."
        exit 1
    }
