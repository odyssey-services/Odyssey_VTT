...
- 2026-08-14 UTC - Doc-sync commit `2292a5339fc5edab54d89621bd5d25d94062c81d` synchronized `docs/tasks/active/ODY-S00-009_Windows_Development_Build_and_Player_Smoke.md` Section 17 and this ExecPlan with the verified clean-HEAD `d8c8431` Player build/smoke evidence. At explicit owner request, Draft PR [#14](https://github.com/odyssey-services/Odyssey_VTT/pull/14) was opened on `feat/ody-s00-009-windows-player-build-smoke` against `main`, head SHA `2292a5339fc5edab54d89621bd5d25d94062c81d`. The PR remains Draft; it is not marked Ready for Review and no merge is requested. ODY-S00-009 status moved to In Review. Known follow-ups carried into the PR body and not blocking Draft: `TC-PLAYER-008` local build-log redaction (retained smoke evidence itself is clean) and an independent re-check of `TC-PLAYER-010`.
- 2026-08-19 UTC - Closed the two remaining PR #14 open items within the approved correction scope (`scripts/build-dev.ps1`, `scripts/test-player-smoke.ps1`, task/ExecPlan evidence only; no `Assets/**`, `Packages/**`, or `config/**` changes). `TC-PLAYER-008`: root cause was the retained Unity Editor build log (`Logs/ODY-S00-009/build-dev-<BuildId>.log`), which a real prior run showed contained the local Windows username 11 times, the absolute repo root path 106 times (both path-separator forms), and the local user-profile path once; no persistent device identifier pattern was found (only an ephemeral per-run Editor connection `[Guid]`/`[EditorId]`). `scripts/build-dev.ps1` now redacts that log in place immediately after the Unity Editor process exits, substituting the exact current username, machine name, repository root, and `$env:USERPROFILE` with fixed placeholders while leaving all other content (including fatal-error lines) untouched; `scripts/test-player-smoke.ps1` now asserts that redaction via `Assert-BuildLogRedacted`, reusing the existing `Test-SafeText` check rather than duplicating it. `TC-PLAYER-010`: `scripts/build-dev.ps1` now runs `Assert-NoDistributionOutputs` against the final artifact tree (extension/naming check for `.msi`/`.msix`/`.pfx`/SBOM/installer-updater-exe/telemetry patterns) after checksums are written and before success is declared; this is additive to, not a duplicate of, the existing keyword-path containment in `test-player-smoke.ps1` `Assert-ArtifactSafety` and the Editor-side canonical single-executable-path guarantee. Both corrections were proven via a new `scripts/build-dev.ps1 -SelfTest` mode using synthetic temp fixtures (`SELFTEST-008`, `SELFTEST-010`, both PASS; fixtures cleaned up in `finally`), independently of a Unity build. Pre-commit validation reconfirmed: `dotnet build`/`dotnet test` (88/88), `verify-format.ps1`, `verify-test-structure.ps1`, `check-repository-policy.ps1`, `verify-repository.ps1` all PASS; `Tests/Metadata/test-catalog.json` was deliberately left unchanged because `scripts/verify-test-structure.ps1` (out of this correction's scope) hard-enforces `TC-PLAYER-008 -> scripts/test-player-smoke.ps1` and `TC-PLAYER-010 -> scripts/build-dev.ps1`, both already correct. PR #14 remains Draft; not merged, not moved to Ready for Review.
- 2026-08-19 UTC - Created the `ODY-S00-010` (SLICE-00 Acceptance and M1 Closure) task contract at `docs/tasks/active/ODY-S00-010_SLICE_00_Acceptance_and_M1_Closure.md` in `Brief plan` mode per backlog. The contract reconciles `ODY-S00-001` through `ODY-S00-009` completion evidence, defines the traceability matrix and quality report scope, and requires a mandatory full clean-checkout rehearsal (fresh `git clone`, all repository scripts, Unity batchmode compile/EditMode/PlayMode, full Windows Player build and smoke) per explicit owner confirmation. It records an unresolved bookkeeping discrepancy found during authoring: `docs/tasks/completed/ODY-S00-004_Identity_Version_and_Result_Primitives.md` still reads `Status: In Review` and records no merge commit for its own PR #8, despite already being Done per this ExecPlan; resolving that is in scope for `ODY-S00-010` implementation. Status remains `Draft`; no rehearsal or reconciliation work has started. Awaiting owner review before activation to `Ready`.
- 2026-08-19 UTC - `ODY-S00-010` reviewed and approved by owner (PR #17 owner-merged, merge commit `db31f91ba48c6883e76e69dac664b9363c84c2f9`), activated to `Ready`. No changes were requested to the contract's Goal, Scope, Acceptance criteria, or other sections 1-15; a decision-log entry recording the approval was added to the task file. Rehearsal execution has not started and is not scoped by this activation.
...
Next action: Begin ODY-S00-010 rehearsal execution (fresh clone, full validation suite, Unity batchmode, Player build/smoke, traceability matrix, quality report) only after this is separately scoped.
