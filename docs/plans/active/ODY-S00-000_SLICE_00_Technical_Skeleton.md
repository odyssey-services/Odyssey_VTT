...
- 2026-08-14 UTC - Independent pre-PR audit of ODY-S00-008 implementation HEAD `eea58a01e63179061f21ad44fabbc27a1a1c880d` returned `NO-GO` for one blocker: the CI workflow/verifier still used the earlier single `odyssey-fast-ci` job shape. Correction scope is limited to `.github/workflows/ci.yml`, `scripts/verify-ci.ps1`, `Tests/Metadata/test-catalog.json`, this ExecPlan, and the active ODY-S00-008 task evidence. The workflow now uses `name: ci`, `pull_request`/`main` plus `push`/`main`, and four `windows-2022` jobs/check contexts: `repository-policy-format-structure`, `dotnet-restore-build-test`, `unity-project-package-static`, and `buildidentity-provenance`. Real GitHub Actions run remains `Pending Draft PR`; branch protection/ruleset remains `Owner action/evidence pending`; PR remains `Not opened`.
...
- ODY-S00-008 implementation evidence: task contract `docs/tasks/active/ODY-S00-008_Fast_CI_and_Build_Identity.md` is In Progress on `feat/ody-s00-008-fast-ci-build-identity`; Personal-license CI decision is recorded as no-secret GitHub Actions plus mandatory local Unity merge validation; no PR is opened.
- ODY-S00-008 CI-contract correction evidence: the workflow now uses `ci` with `pull_request`/`main` and `push`/`main`, four `windows-2022` jobs/check contexts, and scoped artifact upload. Validation passed for restore, verify-format, verify-test-structure, test-fast, dotnet build/test, verify-ci with controlled negative workflow fixtures, verify-unity-project, check-repository-policy, verify-repository, synthetic PullRequest and Development BuildIdentity generation/verification, and local Unity compile/EditMode/PlayMode. Unity batchmode ProjectSettings whitespace drift was restored. A repeat independent pre-PR audit remains required before Draft PR GO.
...
- ODY-S00-002 through ODY-S00-007 are Done. ODY-S00-008 is the current In Progress child task on `feat/ody-s00-008-fast-ci-build-identity`; the CI licensing decision is recorded; no pull request is opened yet.
