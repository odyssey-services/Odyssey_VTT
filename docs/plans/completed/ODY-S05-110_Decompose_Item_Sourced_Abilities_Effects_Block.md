...
- `dotnet test` initially failed two unrelated pre-existing tests (`SqliteBackupRepositoryTests`/`SqliteSavingPipelineTests`'s own kill-harness tests) because the harness DLLs had not been built yet in this fresh worktree — resolved by running `dotnet build` first, matching the standing full-solution workflow; both tests then passed with the identical 534/534 total this docs-only diff should produce.
- Diffed the §14 quote of `ADR-027` §8.1/§8.2 line-by-line against the actual ADR source text (not just visual inspection) and confirmed it byte-for-byte identical.

## 9. Validation and acceptance evidence

- `dotnet build DotNet\Odyssey.Core.sln`: PASS, 0 warnings, 0 errors.
- `dotnet test DotNet\Odyssey.Core.sln`: PASS — Contracts 1/1, Domain 80/80, Networking 67/67, Unit 136/136, Architecture 2/2, Persistence 534/534.
- `.\scripts\verify-format.ps1`: PASS.
- `.\scripts\check-repository-policy.ps1`: PASS.
- `.\scripts\verify-test-structure.ps1`: PASS, exit code 0.
- Diff review: `git diff --name-status` confirmed only §5's allowed paths changed; PR #134 opened as Draft.
