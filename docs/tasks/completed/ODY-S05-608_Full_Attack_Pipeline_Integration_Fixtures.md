...
## 1. Task identity
`ODY-S05-608`; status: Done (PR #153, merged into main).
...
**Product-owner decision (recorded verbatim, 2026-09-16):** the product owner independently verified `608`'s own PR (`#153`) matches this contract, confirmed the zero-production-code composition scope and all eight tests, and separately confirmed the disclosed `CombatEffectExpiryService.EvaluateAndExpireIfDue` wiring gap named above -- unlike `ODY-S05-610`'s own stacking-conflict gap, this one had no reserved future task yet. Decision: merge `ODY-S05-608` as implemented, and reserve a new task, `ODY-S05-611` ("Combat Duration Expiry Automatic Trigger Wiring", see `docs/tasks/active/ODY-S05-611_Combat_Duration_Expiry_Automatic_Trigger_Wiring.md`), to close it -- the same `604`→`609` and `606`→`610` precedent this backlog already established for the delta-commit and stacking-conflict gaps. `ODY-S05-611` depends on `ODY-S05-605` (the existing `EvaluateAndExpireIfDue`/`ActiveEffectExpiryRules`, unmodified) and `ODY-S05-602` (`CombatEncounterService`, the most plausible but not mandated call-site owner); it gives the expiry check a real automatic invocation point, it does not revise `605`'s own pure boundary-check functions or reopen `606`'s effect-application territory.
