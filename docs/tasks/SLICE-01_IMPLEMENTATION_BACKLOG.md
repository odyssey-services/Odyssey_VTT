...
|---:|---|---|---|---|---|---|---|
| 1 | `ODY-S01-007` | Campaign Storage | Campaign Storage Foundation | In Review | None | ExecPlan | Create/open a local campaign: `campaign.db` per `ADR-011` §7 PRAGMA profile, `manifest.json`, `CampaignId`/`CampaignPublicId`, campaign settings, base schema tables, application-level repositories |
