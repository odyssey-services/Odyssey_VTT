...
**Status:** `SLICE-01` CLOSED — owner-accepted 2026-08-25 (see section 3.1). All 8 tasks (`ODY-S01-007`–`014`) are `Done`, merged into `main`: `ODY-S01-007` PR #28; `ODY-S01-008` PR #29; `ODY-S01-009` PR #31; `ODY-S01-010` PR #32; `ODY-S01-011` PR #33; `ODY-S01-012` PR #34; `ODY-S01-013` PR #35; `ODY-S01-014` PR #36. All 8 roadmap §10.6 exit criteria are confirmed with real evidence (`docs/tasks/completed/ODY-S01-014_Traceability_and_Quality_Report.md`); `Milestone M2 — Persistent Local Campaign` (roadmap §10.7) is closed. The product owner explicitly accepted `ODY-S01-014`'s report as-is on 2026-08-25, and PR #36 — which carries that acceptance record — is merged into `main`.
...
**Sequencing note:** this acceptance record shipped in `ODY-S01-014`'s own pull request (`#36`), now merged into `main` (confirmed via `gh pr view 36`). The `CLOSED` status above reflects the owner's decision, made on 2026-08-25 and now in full effect on `main`.
...
| 2 | `ODY-S01-008` | Campaign Storage | Scene and Token Minimal Model | Done (PR #29, merged into main) | 007 | ExecPlan | One `Scene`, two `SceneObject`/token records, asset manifest entry — the minimal domain model roadmap §10.5 steps 3–5 require |
| 3 | `ODY-S01-009` | Saving | Saving Pipeline | Done (PR #31, merged into main) | 007, 008 | ExecPlan | Domain Event Store writes, the single-transaction journal↔projection commit rule (`ADR-012` §5), atomic significant-operation writes, safe application close, crash/unclean-shutdown recovery (WAL replay), open-time integrity check (`05_Persistence` §22) |
| 4 | `ODY-S01-010` | Migration | Migration Registry Baseline | Done (PR #32, merged into main) | 007 | Brief plan | `SchemaHistory` table, `DatabaseSchemaVersion` wiring, one registered `0001_Initial` identity migration with a version and a test — **not** the full `ADR-013` runner (see section 2.1) |
| 5 | `ODY-S01-011` | Backups | Backups | Done (PR #33, merged into main) | 009 | ExecPlan | Manual backup, backup at test-session start/end, snapshot creation via SQLite Backup API (`ADR-012` §8), recent/daily/weekly rotation baseline, restore-into-separate-copy flow, a corruption test fixture |
| 6 | `ODY-S01-012` | Export baseline | Export Baseline | Done (PR #34, merged into main) | 007, 011 | ExecPlan | Initial `.odcamp` container export (`ADR-011`), manifest validation on import, import into a new local copy, explicit absence of automatic merge |
| 7 | `ODY-S01-013` | Integration | Vertical Slice Integration | Done (PR #35, merged into main) | 007–012 | Brief plan | The roadmap §10.5 nine-step scenario as an automated, reproducible end-to-end check: create campaign → import one test map → create scene → place two tokens → move them → close application → reopen campaign → verify saved state → restore from backup |
| 8 | `ODY-S01-014` | Gate | SLICE-01 Acceptance and Closure Gate | Done (PR #36, merged into main) | 007–013 | Brief plan | Traceability matrix, quality report, all eight roadmap §10.6 exit criteria checked with real evidence, owner acceptance — mirrors `ODY-S00-010`'s closure pattern |
