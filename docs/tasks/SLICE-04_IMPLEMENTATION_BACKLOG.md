...
| 13 | `ODY-S04-113` | Done (PR [#97](https://github.com/odyssey-services/Odyssey_VTT/pull/97)) | `CAP-SLICE-05`; `ADR-025` §7.1–7.5 | Character Ruleset Migration | 101, 107 | ExecPlan | `PreviewCharacterRulesetMigration` (read-only Query building `CharacterRulesetMigrationPlan` via new `RulesetMigrationRules`) and `ApplyCharacterRulesetMigration` (one transaction, `PreviewHash`-based stale/tamper detection per `CAP-INV-004`; failure-before-commit rolls back via ordinary `ADR-012` atomicity); `RevertCharacterRulesetMigration` reuses `ODY-S04-107`'s exact compensating-batch pattern (found by the original migration's own `CommandId`, no new ledger table). Explicitly distinct from `ADR-013`'s schema migration runner (confirmed by a direct regression test). Verified fact: no cross-Ruleset-version value/definition-mapping computation exists anywhere in `Packages/com.odyssey.rules/Runtime/` today — this task proves the Preview/Apply/revert *mechanism* honestly against an identity/no-op mapping plus `UnresolvedDecisions` for anything unmappable, per `ADR-025` §7.2's own sanctioned empty-mapping outcome; it does not invent a general transformation algorithm. 12 new tests (`TC-CHAR-153`–`164`), 6 new error codes. |
