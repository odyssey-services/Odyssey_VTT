...
| 4 | `ODY-S04-104` | Not started | `CAP-SLICE-02`; `ADR-023` §7 | Draft Submit/Review/Approve Workflow | 103 | ExecPlan | `SubmitCharacterDraft`, `AddCharacterReviewComment` (conflict-free append, no section revision required), `ApproveCharacterDraft` (`Character.Approve`, MainGM-only under `ADR-019`'s baseline) transitioning `ApprovalState: Draft -> Approved`/`LifecycleStatus: Draft -> Active` on the same `CharacterId` `ODY-S04-103` created. No `Reject`/`ChangesRequested` state (`ADR-023` §7.4). |
