...
| 5 | `ODY-S04-105` | Not started | `CAP-SLICE-03`; `ADR-024` §4–5 | `DevelopmentPool` & Attribute Purchases | 101, 104 | ExecPlan | `DevelopmentPool` as `Mechanics`-section ledger data (`ADR-024` §4), `GrantDevelopmentPoints` (MainGM-only), `PurchaseAttributeIncrease` with `AttributeValue`/`EffectiveValue` computation, Ruleset-driven cost/cap, one-transaction pool+entry+event+ledger commit, `CommandId`/`AppliedCommands` as the sole duplicate-spend guard (`ADR-024` §5). |
