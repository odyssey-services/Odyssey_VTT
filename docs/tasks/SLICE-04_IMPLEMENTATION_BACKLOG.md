...
| 9 | `ODY-S04-109` | Not started | `CAP-SLICE-04` | `CharacterResource` & `AnatomyProfile` | 101 | ExecPlan | Typed `CharacterResource` with computed effective maximum and typed `RecoveryRule` (including `None` = no automatic recovery), maximum-reduction correctly clamping current without a later increase restoring the lost current; `AnatomyProfile` snapshot decoupled from its source profile definition, individual anatomy modifications journaled, dependency preview before removing a body part with a dependent item. |
