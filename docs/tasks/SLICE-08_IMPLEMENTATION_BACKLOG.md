...
**Status:** Implementation revision — OPEN. Block 1 (asset content read API + minimal texture rendering) is merged (`ODY-S08-101`); four blocks remain named, not scoped (camera pan/zoom, drag-and-drop, file picker, token z-order/scale/selection).
...
**Parent task:** `docs/tasks/completed/ODY-S08-101_Asset_Content_Read_And_Texture_Rendering.md`
...
| Order | Block | Status |
|---:|---|---|
| 1 | Asset content read API + minimal texture rendering (scene background and token portraits drawn as images, no camera/pan/zoom -- the same fixed transform as today) | Done (`ODY-S08-101`, section 9 row 1) |
