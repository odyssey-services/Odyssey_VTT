...
**Status:** Implementation revision — OPEN. Block 1 (Application layer for character advancement + decoupling the two "light" methods) is In Review (`ODY-S09-101`); four blocks remain named, not scoped (cost methods, respec, ruleset migration, architecture-check tightening).
...
| 2 | Decoupling the cost methods (`PurchaseAttributeIncrease`, `PurchaseSkillLevel`, `RequestSkillAdvancedRecommendation`, `AcquireAbility`/`AcquireAbilityViaProgressionPurchase`) -- real legality/balance decisions move into the Application service | Named, reserved (section 8) |
...
- **Block 2 -- Cost methods.** Named by this task; not scoped. `PurchaseAttributeIncrease` (attribute cap + cost), `PurchaseSkillLevel` (recommendation gate + cost), `RequestSkillAdvancedRecommendation` (reserved amount), `AcquireAbility` via its private `AcquireAbilityViaProgressionPurchase` (cost). Five Rules call sites today; they are real legality decisions and move into the Application service.
