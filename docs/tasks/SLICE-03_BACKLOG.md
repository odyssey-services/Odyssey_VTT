...
|---:|---|---|---|---|---|---|
| 1 | `ODY-S03-001` | ADR: Board Geometry and Movement Determinism | Draft | None | Not yet determined | Deterministic grid/hex coordinate math, distance metrics, LOS/cover-intersection algorithm, geometry epsilon, and spatial-index approach fixed as `Accepted` — the exact set of decisions `08_Scenes_And_Board` sections 13.4/25.1 name as "implementation ADR," not covered by `ADR-002`/`ADR-017`, which fix command/event and delivery mechanics but not geometry math itself |
