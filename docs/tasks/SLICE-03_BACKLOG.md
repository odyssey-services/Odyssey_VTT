...
|---:|---|---|---|---|---|---|
| 1 | `ODY-S03-001` | ADR: Board Geometry and Movement Determinism | In Review | None | ExecPlan | `docs/adr/ADR-020_Board_Geometry_And_Movement_Determinism_v1.0.md` — `Accepted` in-file. Fixes cross-platform IEEE-754 double arithmetic, exact Square/Hex/None distance formulas, `GeometryEpsilonV1` intersection convention (fail-closed), and `SpatialIndexV1` (uniform spatial hash) — the exact set of decisions `08_Scenes_And_Board` sections 13.4/25.1 name as "implementation ADR," not covered by `ADR-002`/`ADR-017`, which fix command/event and delivery mechanics but not geometry math itself. Draft PR [#55](https://github.com/odyssey-services/Odyssey_VTT/pull/55), all required CI checks green on both commits. Status changes to `Done` only after product-owner review, per this row's own change-control rule — CI-green half of that gate is now satisfied. |
| 2 | `ODY-S03-002` | ADR: Extended Audience and Selected-Participant Visibility | Draft | None | Not yet determined | Fixes `SelectedParticipants`/`CampaignUserGroup`-based audience selection for board fog, dice-roll visibility, and game-log disclosure — extends `ADR-019` section 10's own explicitly-deferred scope (`CampaignUserGroup`, arbitrary `PermissionKey`/`Scope`), which the roadmap section 12.2's own fourth prerequisite bullet ("правила visibility броска, включая selected users/groups") names as still open |
