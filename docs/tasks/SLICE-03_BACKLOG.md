...
**Status:** Prerequisite backlog — **CLOSED** (owner-accepted 2026-08-26; see section 2.1). Both tasks (`ODY-S03-001`, `ODY-S03-002`) are `Done`; both ADRs (`ADR-020`, `ADR-021`) are `Accepted`. `SLICE-03_IMPLEMENTATION_BACKLOG.md` (created by `ODY-S03-003`) is the successor revision.
...
Both of section 2's criteria are satisfied as of `ODY-S03-002`:

1. `ADR-020` (Board Geometry and Movement Determinism) — `Accepted`. Fixes cross-platform IEEE-754 `double` arithmetic, exact Square/Hex/None distance formulas, `GeometryEpsilonV1` (fail-closed intersection convention), and `SpatialIndexV1` (uniform spatial hash) — the two questions `08_Scenes_And_Board` sections 13.4/25.1 explicitly named as "implementation ADR." No technical spike — every decision is provable by golden-vector/brute-force-reference tests per the ADR's own Definition of Done, per section 3's already-established reasoning, not reopened here.
2. `ADR-021` (Extended Audience and Selected-Participant Visibility) — `Accepted`. Fixes `CampaignUserGroup` as a narrow read-model (not a full lifecycle contract), integrates `SelectedParticipants`/`CampaignUserGroup` as additional `VisibilityPolicy` inputs atop `ADR-019` section 7's already-accepted pipeline, and composes postfactum disclosure/revocation with `ADR-017`'s existing `AddJournalEntry`/`AddEntity`/`RemoveFromProjection` operations — extends `ADR-019` section 10's explicitly-deferred scope, the exact gap roadmap section 12.2's fourth prerequisite bullet names. No technical spike — this extends an already `SP-04`-proven redaction mechanism with one new input dimension, per section 3's already-established reasoning, not reopened here.

**Owner acceptance recorded 2026-08-26:** the product owner merged both PR [#55](https://github.com/odyssey-services/Odyssey_VTT/pull/55) (`ODY-S03-001`/`ADR-020`) and PR [#56](https://github.com/odyssey-services/Odyssey_VTT/pull/56) (`ODY-S03-002`/`ADR-021`) — explicit acceptance of both ADRs as `Accepted`, with all required CI checks green on every commit of both pull requests before merge. This closes the `SLICE-03` prerequisite revision. Per this closure task's own scope (`ODY-S03-003`), it does **not** by itself begin `SLICE-03`'s vertical-slice implementation (roadmap section 12.6) — that is `SLICE-03_IMPLEMENTATION_BACKLOG.md`, created by this same task as a separate successor document.

Unlike `SLICE-02_BACKLOG.md`'s closure (which carried forward `ADR-016` section 14's unresolved empirical gate on `SP-03`'s partial coverage), this closure carries forward **no unresolved gap**: both ADRs were accepted on pure architectural/product-documentation grounds, with no deferred spike, no partial-confidence caveat, and no gate blocking any successor task from starting. This is stated explicitly, not assumed, because it is the more unusual (cleaner) outcome relative to this session's own closure precedents, not the default one.
...
| 2 | `ODY-S03-002` | ADR: Extended Audience and Selected-Participant Visibility | Done | None | ExecPlan | `docs/adr/ADR-021_Extended_Audience_And_Selected_Participant_Visibility_v1.0.md` — `Accepted`. Fixes `CampaignUserGroup` as a narrow read-model (not a full lifecycle contract), integrates `SelectedParticipants`/`CampaignUserGroup` as an additional `VisibilityPolicy` input atop `ADR-019` section 7's pipeline, and composes postfactum disclosure/revocation with `ADR-017`'s existing `AddJournalEntry`/`AddEntity`/`RemoveFromProjection` operations — extends `ADR-019` section 10's own explicitly-deferred scope, which the roadmap section 12.2's own fourth prerequisite bullet ("правила visibility броска, включая selected users/groups") names as still open. PR [#56](https://github.com/odyssey-services/Odyssey_VTT/pull/56) merged by product owner (owner review complete) — both this row's change-control conditions (CI green + owner review) now satisfied. |
...
- The `SLICE-03` implementation backlog (vertical slice) is a separate future backlog revision, created only after both ADRs listed in section 4 are `Accepted` — both now are (section 2.1), and `docs/tasks/SLICE-03_IMPLEMENTATION_BACKLOG.md` (created by `ODY-S03-003`) is that successor revision. This document is not rewritten beyond its own closure section (section 2.1) — it remains otherwise a closed, historical artifact, per this repository's convention of not retroactively editing completed backlog revisions.
