...
**Status:** Prerequisite backlog — COMPLETE. All four ADRs (`ADR-022` through `ADR-025`) are `Accepted`. `SLICE-04` vertical-slice implementation may begin as its own separate, not-yet-started future backlog.
...
| 2 | `ODY-S04-002` | ADR: Character Drafts, Templates, and Approval Workflow | Done | 001 | ExecPlan | Accepted `ADR-023` (`docs/adr/ADR-023_Character_Drafts_Templates_And_Approval_Workflow_v1.0.md`). Fixes local Draft vs campaign-authoritative Character boundary (drawn at `BindDraftToCampaign`), personal/campaign template lifecycle as one `CharacterTemplate` aggregate with `TemplateScope`, the deep-copy independent-copy mechanism, compatibility validation/`RulesetVersion` pinning at bind time, and the minimum submit/review/comment/approve command/event flow reusing `ADR-002`/`ADR-019`/`ADR-022` without redefining them. |
| 3 | `ODY-S04-003` | ADR: Development Economy and Progression Transactions | Done | 001 | ExecPlan | Accepted `ADR-024` (`docs/adr/ADR-024_Development_Economy_And_Progression_Transactions_v1.0.md`). Fixes `DevelopmentPool`/`DevelopmentTransaction` as `Mechanics`-section ledger data inside the `ADR-022` Character aggregate, purchase atomicity/duplicate-spend prevention via `ADR-002`/`ADR-012` alone, reservation limited to genuinely pending recommendation workflows, compensating revert/respec via `ADR-012`'s compensating-event mechanism, and `CriticalSuccessEvidence` single-use via a flag-plus-revision on the evidence row itself. |
| 4 | `ODY-S04-004` | ADR: Character Ownership, Lifecycle, and Ruleset Migration Operations | Done | 001, 002, 003 | ExecPlan | Accepted `ADR-025` (`docs/adr/ADR-025_Character_Ownership_Lifecycle_And_Ruleset_Migration_Operations_v1.0.md`). Fixes `CharacterOwnership` inside `ADR-022`'s already-reserved `Ownership` section with `Character.ManageOwnership`-gated (MainGM-only) assignment/co-owner/control-grant commands, dependency-checked physical delete preserving history via `ADR-022`'s event snapshots, Dead/`CharacterRestored` invariants on the `Lifecycle` section, and Character Ruleset migration as a `Preview`/`Apply` workflow distinct from `ADR-013`'s schema migration runner, reusing `ADR-024`'s compensating-batch pattern and `ADR-023`'s Draft-binding pipeline for `.odchar` import. This closes all four `SLICE-04` prerequisite ADR exit criteria (§2). |
