...
|---:|---|---|---|---|---|---|
| 1 | `ODY-S04-001` | ADR: Character Aggregate, Section Revisions, and History Projection | Draft | None | ExecPlan expected | Future `ADR-022`. Fixes Character aggregate boundary, section revisions/locks, lifecycle field ownership, minimum event snapshots, and `CharacterHistoryProjection` as a derived projection, reusing `ADR-002`/`003`/`012`/`013` rather than redefining them. |
