...
|---:|---|---|---|---|---|---|---|
| 1 | `ODY-S02-009` | 1–3 | Identity & Session Admission | Draft | None | Not yet determined | Dev/mock `UserId` assignment wired into a real (not test-only) admission flow; session directory/join-code minimal structure (`06_Networking` §6.3); Lobby state machine; `RolePreset` assignment restricted to the three `ADR-019` baseline roles (MainGM/Player/Observer) — all over `InProcessSessionTransport` |
| 2 | `ODY-S02-010` | 4 | Scene Snapshot & Redacted Projection Delivery | Draft | 009 | Not yet determined | Real `Odyssey.Application`-layer `ProjectionSnapshot`/`VisibilityPolicy`/`PermissionDecision` implementation (per `ADR-017`/`ADR-019`, not a reuse of `ODY-S02-007`'s test-only harness — section 2.3); scene assignment delivered to a newly admitted Player, correctly redacted |
| 3 | `ODY-S02-011` | 5–7 | Authoritative Command & Delta Broadcast | Draft | 010 | Not yet determined | Token-move command validated host-side (`ADR-002` pipeline + `ADR-019` action check, both points from `ADR-019` §6.1); `ProjectionDeltaBatch` broadcast to multiple connections; both clients converge to the same authoritative result |
| 4 | `ODY-S02-012` | 8–10 | Reconnect, Delta Continuity & Duplicate-Delivery Idempotency | Draft | 011 | Not yet determined | Real reconnect flow (`ADR-017` §9's 10 steps), permission recheck by current (not saved) state at reconnect (`ADR-017` §1 point 8), delta-buffer-vs-full-snapshot fallback (`ADR-017` §8), duplicate-batch dedup by `SequenceFrom`/`SequenceTo` (`ADR-017` §6) — satisfies exit criteria 3 and 4 |
