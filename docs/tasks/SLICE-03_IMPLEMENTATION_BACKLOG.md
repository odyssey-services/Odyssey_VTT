...
| 3 | `ODY-S03-006` | 7 | Audience-Aware Roll & Log Delivery | Done | 005 | ExecPlan | New `Odyssey.Application.Dice.DiceRollVisibilityPolicy` (not a `VisibilityPolicy` extension — an incompatible sibling vocabulary per `ADR-021` §3.3) applies `ADR-021` §5's audience model to `DiceRoll` delivery for all four `09_Dice_And_Game_Log` §16 audience kinds (`Public`/`PlayerAndGM`/`GMOnly`/`SelectedParticipants`); new shared `Odyssey.Application.Audience` namespace with a minimal `ICampaignUserGroupDirectory` fixture (`ADR-021` §4's narrow read-model, not the full lifecycle); pure `TryGetVisibleRoll`/`ComputeAudienceViews` functions, no wire codec — satisfies exit criterion 4. 6 new `TC-DICE-019`–`023` tests, all passing, including safe-denial and MainGM-always-sees coverage. PR [#60](https://github.com/odyssey-services/Odyssey_VTT/pull/60) merged by product owner. |
| 4 | `ODY-S03-007` | 8–9 | Game Log & Board State Persistence, Reconnect Replay | Done | 004, 005, 006 | ExecPlan | New `Odyssey.Application.Persistence.IGameLogRepository`/`Odyssey.Persistence.Sqlite.SqliteGameLogRepository` (mirroring `ISceneRepository`/`SqliteSceneRepository`'s exact split, reusing the shared `SqliteSavingPipeline`) persist `DiceRoll`/`GameLogEntry` durably via `ADR-012`'s one-transaction commit; "reconnect" in this task's own campaign-persistence sense (reopening a campaign after a process restart, explicitly NOT `ODY-S02-012`'s networked reconnect protocol — `ADR-017` §3.1's terminology split); new `Odyssey.Application.GameLog.GameLogReconnectService` reuses `ODY-S03-006`'s `DiceRollVisibilityPolicy` unmodified to re-evaluate visibility by CURRENT, not saved, group membership at reopen time — satisfies exit criteria 1 and 5. 4 new `TC-PERSIST-032`–`035` tests, all passing, including a revoked-permission-before-reconnect safe-denial scenario. PR [#61](https://github.com/odyssey-services/Odyssey_VTT/pull/61) merged by product owner. |
| 5 | `ODY-S03-008` | 1–10 | Vertical Slice Integration | Done | 004–007 | Brief plan | The roadmap §12.6 ten-step scenario as one automated, reproducible end-to-end test (`TC-PERSIST-036`), zero new production code: player selects own token via control ownership (`BoardMovementService`) → sends roll intent, host validates permission and generates a formula result via real RNG (`DiceRollService`) → modifiers applied → GM overrides with a mandatory reason → only permitted participants receive the result under a nontrivial `SelectedParticipants` audience with safe denial (`DiceRollVisibilityPolicy`) → event persisted via real SQLite (`SqliteGameLogRepository`) → a reopened repository restores the visible journal by CURRENT, not saved, group membership (`GameLogReconnectService`) → after a full reroll the original event's own data and persisted row remain unchanged. No composition gap found — all ten steps passed on the first real run against already-merged `004`–`007` code. PR [#62](https://github.com/odyssey-services/Odyssey_VTT/pull/62) merged by product owner. |
| 6 | `ODY-S03-009` | — | SLICE-03 Acceptance and Closure Gate | In Review | 008 | Brief plan | Traceability matrix, all eight roadmap §12.7 exit criteria (including `GATE-B` closure) checked with real, re-run evidence — 8 of 8 Pass; criterion 4 ("roll visibility применяется на сетевой границе") carries an explicit, non-blocking scope note (Application-layer boundary proven; no wire-level test exists, by this revision's own deliberate §2.3 scope decision, not an unresolved gap). Owner acceptance pending — mirrors `ODY-S02-015`'s closure pattern. Draft PR (link recorded once opened). |
...
`ODY-S03-004`–`009` task contract files now all exist under `docs/tasks/active/` (created and activated one at a time as each was picked up, per this backlog's original scaffold-only intent).
