...
**Status:** Implementation revision — `ODY-S03-004` In Review (first child task activated); `ODY-S03-005`–`009` Draft, not yet activated
...
|---:|---|---|---|---|---|---|---|
| 1 | `ODY-S03-004` | 1 | Board Foundation: Scene, Token Selection & Authoritative Movement | In Review | None | ExecPlan | Single active scene existence; token selection tied to control ownership (`ControllerUserId`); `BoardMovementService.MoveToken` validated host-side reusing `ADR-002`'s pipeline and `ADR-020`'s `GridType=None` geometry (`BoardGeometry`); `ISceneRepository.MoveToken` now enforces `ExpectedRevision` atomically (a real gap fixed, not previously checked); `UndoMoveToken` as a permission-and-revision-rechecked compensating command, not a blind rollback — satisfies exit criteria 2 and 7. 13 new `TC-BOARD-*` tests, all passing. Draft PR [#58](https://github.com/odyssey-services/Odyssey_VTT/pull/58). |
| 2 | `ODY-S03-005` | 2–6, 10 | Dice Roll Engine, Host Authority, Modifiers & Reroll/Cancel | Draft | None | Not yet determined | Roll intent submission and host-side permission validation (`ADR-019`'s action-check pipeline extended to `Roll.*` keys); d100/formula-parser result generation using `ADR-008`'s host-authoritative injectable RNG; modifier proposal/decision pipeline with no hidden GM numeric modifiers; `GMOverride` as an immutable, reasoned record; full reroll and cancellation, original `DiceRoll` never rewritten — satisfies exit criteria 3 and 6 |
| 3 | `ODY-S03-006` | 7 | Audience-Aware Roll & Log Delivery | Draft | 005 | Not yet determined | `VisibilityPolicy` extended per `ADR-021` §5 applied to `DiceRoll`/`GameLogEntry` delivery — four audience kinds (`Public`/`PlayerAndGM`/`GMOnly`/`SelectedParticipants`) correctly redact roll results at the Application-layer projection-construction point (`ADR-019` §6.2, unchanged) — satisfies exit criterion 4 |
