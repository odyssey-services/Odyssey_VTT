...
|---:|---|---|---|---|---|---|---|
| 1 | `ODY-S03-004` | 1 | Board Foundation: Scene, Token Selection & Authoritative Movement | Draft | None | Not yet determined | Single active scene existence; token selection tied to `LinkedEntityRef`/control ownership; `MoveToken` command validated host-side reusing `ADR-002`'s pipeline and `ADR-020`'s geometry/distance formulas; Undo/Redo for the movement command as a permission-and-revision-rechecked compensating command — satisfies exit criteria 2 and 7 |
