...
**Status:** Implementation revision — OPEN. Content Catalog MVP (`ODY-S05-101`–`106`), Inventory runtime (`ODY-S05-201`–`207`), Equipment runtime (`ODY-S05-301`–`306`), `ItemDefinition` migration (`ODY-S05-401`–`404`), item-sourced abilities/effects (`ODY-S05-501`–`507`), and the full attack pipeline (`ODY-S05-601`–`611`, `ADR-029`) are all complete and merged into `main`.
...
| 4 | `ODY-S05-204` | Done (PR [#116](https://github.com/odyssey-services/Odyssey_VTT/pull/116), merged into `main`) | `ADR-027` §5/7; Domain Model §17.5 | Inventory Move / Transfer MVP | 201, 202, 203 | ExecPlan | MainGM-only contained-to-contained ItemInstance/ItemStack moves with atomic target/Inventory revision guards, persisted destination owner, and separate `InventoryMoveCommandLedger` replay identity. No equip/unequip, drop/pickup, split/merge, attack, item use, or ActiveEffect behavior. |
...
**Block status: complete** (`ODY-S05-401`–`404`, PR #130/#131/#132/#133, all merged into `main`).
...
| 2 | `ODY-S05-603` | Done (PR [#147](https://github.com/odyssey-services/Odyssey_VTT/pull/147), merged into `main`) | `ADR-029` §1 rules 3–4, §5–§6 stages 1–11 | Attack Intent, Preview, and Evaluation | 602 | ExecPlan | Pure preview and host-recomputed Rules evaluation, RNG inputs, and `EffectApplicationDecision`; no persisted apply. |
| 3 | `ODY-S05-604` | Done (PR [#148](https://github.com/odyssey-services/Odyssey_VTT/pull/148), merged into `main`) | `ADR-029` §1 rules 5–6, §6 stages 12–13 | Intervention and Atomic Attack Apply | 602, 603 | ExecPlan | Pending resolution, intervention command, no-reroll/idempotency, and one atomic transaction. |
| 4 | `ODY-S05-605` | Done (PR [#149](https://github.com/odyssey-services/Odyssey_VTT/pull/149), merged into `main`) | `ADR-029` §7 | Combat Effect Duration Expiry | 602, 502 | ExecPlan | Six combat duration boundary mechanisms/bindings; no effect creation or `ADR-028` redesign. |
| 5 | `ODY-S05-606` | Done (PR [#150](https://github.com/odyssey-services/Odyssey_VTT/pull/150), merged into `main`) | `ADR-029` §1 rule 8, §8 | Combat ActiveEffect Application | 603, 604, 605 | ExecPlan | Apply explicit combat effect decisions through existing ActiveEffect saving pipeline. |
| 6 | `ODY-S05-607` | Done (PR [#151](https://github.com/odyssey-services/Odyssey_VTT/pull/151), merged into `main`) | `ADR-029` §1 rule 7/10, §6 stages 14–15 | Compensation and Game Log Projections | 604 | ExecPlan | Causally-linked compensation and redacted committed projections; never rollback history. |
| 7 | `ODY-S05-609` | Done (PR [#152](https://github.com/odyssey-services/Odyssey_VTT/pull/152), merged into `main`) | `ADR-029` §1 rule 5, §6 stage 13, §12 item 4 | Attack Aggregate Delta Commit | 604 | ExecPlan | Applies accepted `AttackDelta` values to Character resource state, atomically inside the attack's own apply transaction (never the public `SetResourceCurrentValue`); item-targeted deltas are a disclosed, escalated blocker (no numeric field exists on `ItemInstanceRecord`). No new Ruleset formula. |
| 8 | `ODY-S05-610` | Done (PR [#154](https://github.com/odyssey-services/Odyssey_VTT/pull/154), merged into `main`) | `ADR-028` §7 rule 7 | Combat Stacking Conflict GM Resolution | 606 | ExecPlan | Durable persistence and a real MainGM resolution command for `RequestGMResolution` stacking conflicts; closes `606`'s own disclosed gap. |
| 9 | `ODY-S05-608` | Done (PR [#153](https://github.com/odyssey-services/Odyssey_VTT/pull/153), merged into `main`) | `ADR-029` §12 | Full Attack Pipeline Integration Fixtures | 602-607, 609 | Brief plan | End-to-end proof of all nine §12 outcomes without new production behavior. |
| 10 | `ODY-S05-611` | Done (PR [#155](https://github.com/odyssey-services/Odyssey_VTT/pull/155), merged into `main`) | `ADR-029` §7 | Combat Duration Expiry Automatic Trigger Wiring | 605, 602 | ExecPlan | Gives `CombatEffectExpiryService.EvaluateAndExpireIfDue` a real automatic call site inside `SqliteCombatEncounterRepository.Advance`; closes `605`'s own disclosed deferred-wiring gap, reconfirmed still open by `608`. Final task of the full attack pipeline block. |
