...
|---:|---|---|---|---|---|---|---|
| 1 | `ODY-S05-101` | Ready | `ADR-027` §4; `11_Content_Block_System` §6/21/22 | Content Catalog Foundation | None | ExecPlan | Storage/contracts for `ContentDefinition` records: Draft/Published/Archived lifecycle state, `DefinitionVersion`/`Revision` fields, immutable Published rows, and exact-definition-version reference shape future runtime code will pin to. Explicit no-runtime-item-state boundary — the catalog stores definitions only, never `ItemInstance`/`ItemStack`/Inventory/equipment/`ActiveEffect` state. Base/Ruleset-scoped only (section 3.2). |
| 2 | `ODY-S05-102` | Ready | Product-owner MVP answer; `ADR-027` §4 | GM Catalog Authoring MVP | 101 | ExecPlan | MainGM-issued commands to create a Draft definition, edit an existing Draft, and create the next Draft version from a Published definition (never editing a Published version in place). Base/Ruleset catalog only — no campaign-specific catalog or per-campaign override mechanism (section 3.2). |
...
| 4 | `ODY-S05-104` | Ready | Product-owner MVP answer; `ADR-027` §4 | Catalog Validation MVP | 101, 105 | ExecPlan | Pre-publish validation proving real usability/applicability, not just required-field presence, for item/weapon/armor/ammo/ability/effect definitions (full per-type expectations in section 6 below); missing-definition-reference checks; `ContentBlock` graph cycle/unsupported-operation rejection; Ruleset/version compatibility checks. Consumed by `103`'s own publish gate. |
| 5 | `ODY-S05-105` | Ready | `ADR-027` §3.1, §4; `11_Content_Block_System` | Base Definition Types | 101 | ExecPlan | Typed catalog definitions: `ItemDefinition`, `WeaponDefinition`, `ArmorDefinition`, `AmmoDefinition`, `AbilityDefinition`, `EffectDefinition`, plus `Resource` and `BodyPart` structural references, with typed properties sufficient for later Inventory/Equipment/Attack tasks to consume (attack properties, protection/durability, ammo compatibility, ability trigger/cost/target, effect duration/stacking) without re-deciding catalog shape when those future tasks are activated. No concrete balanced content. |
