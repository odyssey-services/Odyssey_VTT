...
| 4 | `ODY-S05-104` | In Review (PR [#108](https://github.com/odyssey-services/Odyssey_VTT/pull/108)) | Product-owner MVP answer; `ADR-027` §4 | Catalog Validation MVP | 101, 105 | ExecPlan | `CatalogValidationService` (`ValidateContentDefinition`/`ValidateDraftForPublish`, Application layer): real usability validation via `ODY-S05-105`'s own `TypedDefinitionCodec` for item/weapon/armor/ammo/ability/effect; missing/wrong-version/wrong-type exact-reference rejection with dependency-cycle detection across the real `ContentDefinitionRef` graph; Ruleset/version compatibility check; ContentBlock/mechanics-payload MVP boundary validated structurally (no real `ContentBlockGraph` exists yet). Side-effect-free -- no repository write is ever called. 30 new tests (`TC-CATALOG-042`–`071`), 0 new error codes (validation issues are a plain enum, not `ErrorCode`s). Consumed by `103`'s own future publish gate. |
