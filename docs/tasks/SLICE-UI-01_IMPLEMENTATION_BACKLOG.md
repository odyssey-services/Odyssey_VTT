...
**Status:** Done — `SLICE-UI-01` fully closed: 8 of 8 tasks (`ODY-UI-01-002` through `ODY-UI-01-007`, plus point fixes `007a` and `007b`) are in `main`; the product owner personally confirmed the manual Unity Play Mode scenario.
...
| 2 | `ODY-UI-01-003` | Role selector | Done | None | ExecPlan | A compact UI Toolkit "Playing as: Player / MainGM / Observer" selector backed by shared Client-layer `RoleSelection` state using existing `BaselineRole` values. Supplies `actorUserId`/`actorIsMainGm`/`ActorCanCreateRoll` for later screens and retrofits `BoardScreenPresenter` to follow the selected actor. 4 new EditMode tests passed in a real `scripts/test-unity.ps1` run (44/44 EditMode, 2/2 PlayMode). PR [#70](https://github.com/odyssey-services/Odyssey_VTT/pull/70) merged to `main`; CI run [33094205756](https://github.com/odyssey-services/Odyssey_VTT/actions/runs/33094205756) passed all 4 checks. |
| 3 | `ODY-UI-01-004` | Roll panel + modifiers | Done | 003 | ExecPlan | `RollPanelPresenter` adds a formula field and "Roll" button calling `DiceRollService.SubmitRoll` from the current `RoleSelection`, plus modifier label/value proposal and MainGM Accept/Change/Reject decisions through `ProposeModifier`/`DecideModifier`. Stores the most recent successful `DiceRoll` as `LastRoll` for later UI tasks; default audience is `Public` because audience-aware display is `ODY-UI-01-005`. 6 new EditMode tests passed in a real `scripts/test-unity.ps1` run (50/50 EditMode, 2/2 PlayMode). PR [#71](https://github.com/odyssey-services/Odyssey_VTT/pull/71) merged to `main`; CI run [33105832028](https://github.com/odyssey-services/Odyssey_VTT/actions/runs/33105832028) passed all 4 checks. |
| 4 | `ODY-UI-01-005` | Override + audience-aware result display | Done | 003, 004 | ExecPlan | `RollPanelPresenter` now has a roll audience selector (`Public`, `PlayerAndGM`, `SelectedParticipants`) defaulting to `PlayerAndGM`, plus a MainGM-only override reason field/button calling `DiceRollService.ApplyOverride`. Result display calls `DiceRollVisibilityPolicy.TryGetVisibleRoll` before showing roll details, so Player/MainGM see the default roll and Observer gets safe-denial text with no formula/total details. 5 new/updated EditMode coverage points passed in a real `scripts/test-unity.ps1` run (55/55 EditMode, 2/2 PlayMode). PR [#72](https://github.com/odyssey-services/Odyssey_VTT/pull/72) merged to `main`; CI run [33125908428](https://github.com/odyssey-services/Odyssey_VTT/actions/runs/33125908428) passed all 4 checks. |
| 5 | `ODY-UI-01-006` | Persistence + game log | Done | 004 | ExecPlan | `GameLogPresenter` adds a "Save & Reopen Campaign" action persisting the latest roll through `SqliteGameLogRepository.SaveDiceRollEntry`, then re-listing via a fresh repository instance against the same supplied `CampaignHandle`. It renders an unstyled scrollable list of visible `GameLogEntryRecord.SummaryPayload` values filtered through `GameLogReconnectService.GetVisibleEntries`, so Player/MainGM see the default `PlayerAndGM` roll and Observer does not. 4 new EditMode tests passed in a real `scripts/test-unity.ps1` run (59/59 EditMode, 2/2 PlayMode). PR [#73](https://github.com/odyssey-services/Odyssey_VTT/pull/73) merged to `main`; CI run [33127771319](https://github.com/odyssey-services/Odyssey_VTT/actions/runs/33127771319) passed all 4 checks. |
| 6 | `ODY-UI-01-007` | Reroll/cancel + full walkthrough | Done | 002, 003, 004, 005, 006 | ExecPlan | `RollPanelPresenter` adds Reroll/Cancel buttons calling `RequestFullReroll`/`CancelRoll`; `TrialScreenPresenter` assembles board, one role selector, roll panel, and game log into the existing Bootstrap/AppShell launch path via `Open Trial UI`. Focused reroll/cancel tests and one composed ten-step walkthrough passed in a real `scripts/test-unity.ps1` run (62/62 EditMode, 3/3 PlayMode). `scripts/build-dev.ps1` passed on commit `58ce421` and produced `artifacts/builds/odyssey-development-1787913318.1-g58ce4213832d/Windows-x64/Odyssey.exe`. PR [#74](https://github.com/odyssey-services/Odyssey_VTT/pull/74) merged to `main`; CI run [33164251464](https://github.com/odyssey-services/Odyssey_VTT/actions/runs/33164251464) passed all 4 checks. |
| 7 | `ODY-UI-01-007a` | Runtime UI click routing gap fix | Done | 007 | ExecPlan | Fixes the runtime gap where visible UI Toolkit buttons hovered/focused but did not invoke `Button.clicked` in Unity Play Mode. Adds project UI action corrections, runtime `EventSystem`/`InputSystemUIInputModule` setup, and a real mouse-click PlayMode regression. Initial stacked PR [#75](https://github.com/odyssey-services/Odyssey_VTT/pull/75) merged into the intermediate `007` branch; final mainline inclusion happened through PR [#77](https://github.com/odyssey-services/Odyssey_VTT/pull/77), merge SHA `27b2a1dac67daa91c653ef01593aae02a089340f`. |
| 8 | `ODY-UI-01-007b` | Trial screen layout overlap fix | Done | 007a | ExecPlan | Fixes the Trial screen controls-column overlap found after click routing was restored by linking `AppShell.uss` and adding minimal Roll Panel/Game Log layout rules plus PlayMode geometry coverage. Initial stacked PR [#76](https://github.com/odyssey-services/Odyssey_VTT/pull/76) merged into the intermediate `007a` branch; final mainline inclusion happened through PR [#77](https://github.com/odyssey-services/Odyssey_VTT/pull/77), merge SHA `27b2a1dac67daa91c653ef01593aae02a089340f`. Product owner personally confirmed in Unity Play Mode that clicks work and overlap is gone; remaining text-contrast polish is noncritical and out of scope. |

Final slice status: `SLICE-UI-01` is fully closed — 8 of 8 tasks (`ODY-UI-01-002` through `ODY-UI-01-007`, plus `007a` and `007b`) are merged into `main`, and the product owner personally confirmed the manual scenario in Unity Play Mode.
...
This backlog originally reserved the `ODY-UI-01-002`–`007` numbers, titles, and boundaries. The two point fixes `007a` and `007b` were added after owner manual review found a runtime click-routing gap and a Trial screen layout overlap. Their task contracts are now created under `docs/tasks/active/` and the complete slice is closed.
