...
|---:|---|---|---|---|---|---|
| 1 | `ODY-UI-01-002` | Board screen | In Review | None | ExecPlan | `BoardScreenPresenter` renders the active scene's tokens at their real `TokenPosition` coordinates (`GridType=None`, no hex/grid rendering per `ADR-020`'s own scope); click-to-select then click-destination-to-move calls `BoardMovementService.MoveToken` with a single hardcoded local actor identity; a non-controller's attempted move is denied. Satisfies roadmap §12.6 step 1 and exit criterion 2 ("Player не может перемещать чужой токен без control") by hand. 4 new EditMode tests, correct by code review. **Significant discovered blocker (not this task's own scope to fix):** Unity Editor cannot compile `Odyssey.Persistence` at all (`Microsoft.Data.Sqlite` has no Unity Plugin/`precompiledReferences` equivalent) — pre-existing since `SLICE-01`, confirmed unrelated to this task's own diff; blocks a real run of these tests and will block every subsequent `SLICE-UI-01` task touching persistence until a dedicated follow-up task adds the missing Unity SQLite plugin references. Draft PR [#67](https://github.com/odyssey-services/Odyssey_VTT/pull/67), CI green (static checks only — does not resolve the discovered blocker), awaiting product owner review. |
