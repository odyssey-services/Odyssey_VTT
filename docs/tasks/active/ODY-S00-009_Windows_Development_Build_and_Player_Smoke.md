...
| `TC-PLAYER-008` | Closed | Root cause: the retained Unity Editor build log (`Logs/ODY-S00-009/build-dev-<BuildId>.log`, written by `scripts/build-dev.ps1` via `-logFile`) is repository-defined evidence (referenced from this Section 17) and is not committed, but was found to contain the local Windows username 11 times, the absolute repo root path 106 times (both `\` and `/` forms), and the local user-profile path once, on a real prior run. `scripts/build-dev.ps1` now applies `Protect-EvidenceLogFile`/`Protect-EvidenceLogText` in-place to that log immediately after the Unity Editor process exits and before the artifact is declared built: it substitutes the exact current `[Environment]::UserName`, `[Environment]::MachineName`, the absolute repository root, and `$env:USERPROFILE` (both path-separator forms) with fixed placeholders (`<REDACTED_USER>`, `<REDACTED_HOST>`, `<REPO_ROOT>`, `<USER_PROFILE>`), leaving all other log content (including error/fatal messages) untouched. Persistent device identifiers were investigated in a real prior build log and none were found (only an ephemeral per-run Editor connection `[Guid]`/`[EditorId]`, not a persistent hardware ID); no redaction was added for that category. Player smoke evidence (`smoke-run-*.json`) already contained none of these categories and required no change. Proof: `scripts/build-dev.ps1 -SelfTest` (`SELFTEST-008`) builds a synthetic fixture log containing real username/machine-name/absolute-path markers plus an unrelated fatal-error line, runs the redaction function, and asserts all three marker categories are gone, all four placeholders are present, and the fatal-error line survives verbatim — PASS. Fixtures are created under the OS temp directory and removed in a `finally` block. |
| `TC-PLAYER-010` | Closed | `scripts/build-dev.ps1` now calls `Assert-NoDistributionOutputs` against the final `artifacts/builds/<BuildId>/Windows-x64/` tree after checksums are written and before success is declared, fail-closed on any Release/RC-signing extension (`.msi`, `.msix`, `.msixbundle`, `.appx`, `.appxbundle`, `.pfx`, `.p12`, `.snk`, `.cat`, SPDX/CycloneDX SBOM files), installer/updater-style executables (`Setup*.exe`, `Installer*.exe`, `Uninstall*.exe`, `Updater*.exe`), or filenames containing `sbom`/`telemetry`. This is additive to, and does not duplicate, the existing keyword-path containment in `scripts/test-player-smoke.ps1` `Assert-ArtifactSafety` (`release`/`rc`/`installer`/`updater`/`sbom`/`telemetry` as literal path segments) and the Editor-side single canonical executable path (`OdysseyDevelopmentBuild.ResolveValidatedBuildOutputPath`) — both of which remain unchanged. Proof: `scripts/build-dev.ps1 -SelfTest` (`SELFTEST-010`) confirms a clean synthetic artifact directory (`Odyssey.exe` + `UnityCrashHandler64.exe`, matching real Development-Debug output) passes without a false positive, and a second synthetic directory with an injected `Setup.msi` is correctly rejected — PASS. |
...
- Clean-HEAD Windows Development-Debug artifact and Player smoke evidence exist and are independently verifiable (build identity parity, checksum integrity, smoke pass flags), but this evidence lives only in local, gitignored `artifacts/`/`Logs/` directories and is not part of committed/pushed repository state or PR-visible evidence.
- `TC-PLAYER-008` and `TC-PLAYER-010` are closed on the strength of `scripts/build-dev.ps1 -SelfTest` synthetic-fixture proof (`SELFTEST-008`, `SELFTEST-010`). A full clean-HEAD Windows Player build/smoke run under the corrected script is expected to additionally confirm both in production conditions; its result is recorded in the PR/task validation record once available.
- ODY-S00-009 remains In Review; PR #14 stays Draft pending a separate owner decision to move to Ready for Review.

### Follow-up tasks

- Confirm `TC-PLAYER-008`/`TC-PLAYER-010` production behavior via a full clean-HEAD `scripts/build-dev.ps1` + `scripts/test-player-smoke.ps1` run and, if useful, record its BuildId/evidence in this section.
- Before or alongside PR review, consider recording/attaching clean-HEAD build/smoke evidence (BuildId, checksums, smoke-run JSON) in a PR-visible form, since it currently exists only in local gitignored directories.
- Clean up the empty, apparently failed second build attempt directory (`odyssey-development-1786720710.1-gd8c8431df985`) from a prior run if not needed.
