...
**Created:** 2026-09-17
**Last updated:** 2026-09-16 UTC -- `ODY-S06-106` (row 6) moved to `In Review`; interpreter ownership resolved in section 9.1's own `ODY-S06-106` entry (no conflict with `105`). See also section 10's own `ODY-S06-102` restructuring entry.
...
| 3 | `ODY-S06-103` | In Review | Roadmap block B | Weapon Equip-Status Check + Armor Wiring Into Attack Snapshot | 101 | Not applicable (small enough to implement directly, per this task's own contract §16) | The action weapon must now be currently Equipped, not merely owned (`SqliteAttackStateReader.Read` gains an equip-status gate); `AttackEvaluationSnapshot.ArmorAndEffects` is retyped from the structurally-data-incapable `AttackUnavailableInput` to a new `AttackArmorInput` union, carrying every target's own real, `TypedDefinitionCodec`-decoded `ArmorDefinition`, tagged by `TargetId`, via a new `IInventoryRepository.ListEquippedEntriesByCharacter`. Closes the second data gap blocking `105`. |
| 4 | `ODY-S06-104` | In Review | Roadmap block C | Character↔Token Link + Attack Participant Position/Distance Wiring | 101 | Not applicable (small enough to implement directly, per this task's own contract §16) | `TokenRecord` gains an optional `CharacterId` link (`ISceneRepository.CreateToken`'s own new optional parameter); `AttackEvaluationSnapshot.Topology` is retyped from the structurally-data-incapable `AttackUnavailableInput` to a new `AttackTopologyInput` union, carrying each target's own real Euclidean distance from the actor (`BoardGeometry.EuclideanDistance`, world units/meters -- the same scale `WeaponDefinition.Range` is read in), resolved via a new campaign-wide `ISceneRepository.ListTokensByCharacter`. Closes the third and final data gap blocking `105`. |
| 5 | `ODY-S06-105` | In Review | `ADR-030` §5/§6 | Real `IAttackRulesEvaluator` Implementation + Formula Grammar | 101, 102, 103, 104 | Not applicable (small enough to implement directly, per this task's own contract §16) | `CoreAttackRulesEvaluator` (`Odyssey.Rules.Combat`) -- the first production `IAttackRulesEvaluator`, computing a real formula-driven damage value (a new `AttackDamageFormulaParser`, `ADR-030` §6's own documented `DiceFormulaParser` superset), consulting `102`'s real attributes, `103`'s real armor (summed), and `104`'s real distance (range miss = no damage). `AttackRandomSample` extended from a single int to an ordered `Values` list, resolving the long-open multi-dice-term gap. `AttackEvaluationService` now decodes the action item into a `WeaponDefinition` before calling the evaluator, since `Odyssey.Rules` cannot reference `Odyssey.Application.Content.TypedDefinitionCodec` (ADR-001 §6.2). |
| 6 | `ODY-S06-106` | In Review | `ADR-030` §7/§9 (roadmap block G) | `ActivateAbility` Command | 101 | ExecPlan | The `ActivateAbility` root command (`ADR-030` §9): resolves `CharacterAbilityId` through the content catalog (via `CharacterAbility.SourceRef`, reused additively as the `AbilityDefinitionId`↔`ContentDefinitionRef` bridge -- a genuine architecture gap this task found and resolved), interprets `MechanicsPayloadRef` through the FIRST real implementation of `ADR-030` §7's mechanics-primitive interpreter (`AdjustResource`/`ApplyEffect`, exactly two), and applies the result atomically, outside `CombatEncounter` state per `ADR-030` §9.3's own explicit MVP decision. |
...
**Ownership resolution (recorded here, per this row's own requirement, since `106` is the task whose contract lands second):** `ODY-S06-105` did NOT implement `AdjustResource`/`ApplyEffect` or any payload interpreter -- it only implemented the weapon damage formula grammar (`AttackDamageFormulaParser`) and the dice-sampling extension (`AttackRandomSample.Values`), independently confirmed by direct code read before `106` started. `ODY-S06-106` is therefore the task that builds `ADR-030` §7's `MechanicsPrimitiveEnvelope`/`MechanicsPayloadCodec`/`MechanicsPrimitiveInterpreter` from scratch, reusing (not forking) `105`'s own `AttackDamageFormulaParser`/`AttackRandomSample` for formula/dice resolution. No ownership conflict exists between the two tasks.
