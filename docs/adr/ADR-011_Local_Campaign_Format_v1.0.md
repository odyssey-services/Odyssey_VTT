...
**Закрыто:** `ADR-011_Local_Campaign_Format_v1.1.md` разрешает этот вопрос — `Microsoft.Data.Sqlite` с `SQLitePCLRaw.bundle_e_sqlite3 >= 3.0.3`, на основании рекомендации `docs/tasks/completed/ODY-S01-005_SP-02_Persistence_Reliability_Report.md` §3. Вопрос более не открыт.
...
- открытые вопросы раздела 12 остаются открытыми до отдельного решения и не считаются молчаливо решёнными фактом принятия этого ADR (кроме §12.1, закрытого `ADR-011` v1.1 — см. пометку в §12.1 выше);
...
**ADR-011 v1.0 остаётся историческим контекстом в части §12.1. Active work must use `ADR-011_Local_Campaign_Format_v1.1.md` для выбора SQLite provider-библиотеки; все остальные разделы этого документа (v1.0) остаются нормативными и действующими без изменений.**
