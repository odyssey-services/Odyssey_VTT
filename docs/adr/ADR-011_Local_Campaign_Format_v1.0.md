...
**Источник рекомендации:** `docs/tasks/active/ODY-S01-005_SP-02_Persistence_Reliability_Report.md` §3 содержит эмпирически обоснованную рекомендацию (не решение) по этому вопросу. Статус этого пункта остаётся `[OPEN]` — отчёт не закрывает его сам; закрытие остаётся отдельным шагом по решению владельца продукта.
