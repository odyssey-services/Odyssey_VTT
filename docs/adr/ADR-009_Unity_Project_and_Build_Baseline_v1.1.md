...
**Документ:** `docs/adr/ADR-009_Unity_Project_and_Build_Baseline_v1.1.md`
**ADR:** ADR-009
**Версия:** 1.1
...
1. Точный Unity Editor baseline — **Unity `6000.4.0f1`**, changeset `8cf496087c8f`.
...
- Unity Editor Windows x64 `6000.4.0f1`;
...
PR-001 создаёт проект официальным HDRP template, совместимым с `6000.4.0f1`.
...
Начиная с Unity 6.4 Package Manager проверяет цифровые подписи tarball packages; signature warning/error не игнорируется в CI.
...
| TestCaseId | Сценарий |
|---|---|
| `TST-UNI-001` | ProjectVersion соответствует `6000.4.0f1` и revision |
...
Переход `6000.4.0f1 → 6000.4.x` требует отдельного PR:
...
- Unity documentation по Package Manager signatures начиная с Unity 6.4;
- Unity documentation по build pipeline и Windows/IL2CPP modules.

Current/latest online pages не заменяют pinned 6000.4 documentation и repository lock files.
...
- Unity baseline проекта — `6000.4.0f1 (8cf496087c8f)`;
