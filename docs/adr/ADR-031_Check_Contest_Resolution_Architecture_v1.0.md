...
6. **Degrees of success (critical success/failure beyond plain pass/fail) and advantage/disadvantage (roll twice, take better/worse) are EXPLICIT NON-GOALS for this first version -- product decisions made within this task, not silently deferred.** Section 8 records both as named, disclosed non-goals, exactly the way `ADR-030` §12 disclosed "`ActivateAbility` as a combat-turn-consuming action" as a future extension rather than an oversight. Both decisions are flagged for product-owner confirmation at the first opportunity (section 17) -- they are this task's own best engineering judgment about MVP scope, not a claim of product authority the ADR does not have.
...
## 8.2 Degrees of success: explicit non-goal for this version

This ADR's own first version produces plain pass/fail plus the one natural-maximum-roll signal section 8.1/10 already requires for the `RecordCriticalSuccessEvidence` wiring -- it does NOT produce graduated degrees of success (e.g. "success by 5 or more is an exceptional success," or a distinct "critical failure" outcome for a natural minimum roll) beyond that one flag. This is a product decision made within this task's own engineering judgment, not a silent omission: degrees of success add real design surface (how much margin counts as "exceptional," whether critical failure exists and what it does mechanically) that no product-owner guidance addresses today, and building it speculatively risks guessing wrong about a system the product owner has not yet specified. Flagged in section 17 for product-owner confirmation at the first opportunity.

## 8.3 Advantage/disadvantage and contests: explicit non-goals for this version

Rolling twice and taking the better or worse result (advantage/disadvantage), and opposed contests (section 3.2, one check compared against another instead of against a static DC), are both explicit non-goals for the first implementation task. Advantage/disadvantage is structurally a `DiceRollService`-level concern (which roll of two "counts") this ADR does not design; a contest is architecturally a straightforward extension of section 8.1's own comparison (compare two resolved totals against each other instead of one total against a fixed number) but is not itself built here. Both are named in this ADR's own title and this section specifically so a future task does not have to independently rediscover that the underlying machinery (the shared formula grammar, the `DiceRollService` substrate, the pass/fail comparison shape) already supports extending toward either without redesign -- flagged in section 17 alongside degrees of success for the same product-confirmation reason.
...
- specify degrees of success beyond plain pass/fail plus one natural-maximum-roll signal (section 8.2) -- an explicit, product-confirmation-pending non-goal;
- specify advantage/disadvantage or a dedicated opposed-contest command (section 8.3) -- explicit, product-confirmation-pending non-goals;
...
**Accepted:** plain pass/fail plus the one natural-maximum-roll signal `RecordCriticalSuccessEvidence`'s own existing hook needs (section 8.2/8.3), with both richer mechanics named, disclosed, and flagged for product-owner confirmation (section 17) rather than either silently built or silently dropped.

---

# 17. Открытые вопросы

Two decisions in this ADR are product decisions made within this task's own engineering judgment, not settled product-owner guidance, and are recorded here explicitly rather than silently deferred, per this task's own Definition of Done:

1. **Degrees of success (section 8.2):** this ADR's first version produces plain pass/fail plus one natural-maximum-roll signal only. Whether a richer outcome model (critical failure, graduated success margins) is wanted for the MVP or a later iteration is not yet confirmed by the product owner.
2. **Advantage/disadvantage and dedicated contest support (section 8.3):** this ADR's first version builds neither. Whether either is wanted before or alongside the first check implementation, or genuinely deferred to a later `SLICE-07` (or later) task, is not yet confirmed by the product owner.

Both should be confirmed at the product owner's first opportunity, ideally before `ODY-S07-102`'s own task contract is finalized, so that task does not have to guess whether its own scope is complete. Everything else this ADR's own sections 5-11 decide is an engineering decision, justified by direct precedent (`ADR-030`, `ODY-S06-102`/`105`/`106`/`107`) or direct code inspection, not a deferred product question.
