...
namespace Odyssey.Persistence.Sqlite
{
    ...
    public sealed class SqliteCombatEncounterRepository : ICombatEncounterRepository
    {
        ...
        public Result<CombatEncounterRecord> Create(CampaignHandle campaign, CreateCombatEncounterRequest request, CorrelationId correlationId)
        {
            ...
            try
            {
                ...
                Result<CombatEncounterRecord>? replay = Replay(c, t, campaign.CampaignId, request.CommandId, "create", Fingerprint(request.ParticipantOrder));
        ...
        public Result<CombatEncounterRecord> Advance(CampaignHandle campaign, AdvanceCombatEncounterRequest request, CorrelationId correlationId)
        {
            if (campaign == null || request == null) throw new ArgumentNullException(campaign == null ? nameof(campaign) : nameof(request));
            if (!request.ActorIsMainGm) return Result<CombatEncounterRecord>.Failure(Denied(correlationId));
            try
            {
                ...
                Result<CombatEncounterRecord>? replay = Replay(c, t, campaign.CampaignId, request.CommandId, "advance", fingerprint);
                if (replay != null) { t.Commit(); return replay.Value; }
        ...
        private static bool Update(SqliteConnection c, SqliteTransaction t, CombatEncounterId id, long expected, long revision, long round, long turn, CombatEncounterStatus status, CombatPhase phase, CharacterId? current, UtcInstant now) { using var q = c.CreateCommand(); q.Transaction = t; q.CommandText = "UPDATE CombatEncounter SET Revision=$revision,RoundOrdinal=$round,TurnOrdinal=$turn,Status=$status,Phase=$phase,CurrentParticipantId=$current,UpdatedAt=$now WHERE EncounterId=$id AND Revision=$expected;"; q.Parameters.AddWithValue("$id", id.ToString()); q.Parameters.AddWithValue("$revision", revision); q.Parameters.AddWithValue("$round", round); q.Parameters.AddWithValue("$turn", turn); q.Parameters.AddWithValue("$status", status.ToString()); q.Parameters.AddWithValue("$phase", phase.ToString()); q.Parameters.AddWithValue("$current", current.HasValue ? (object)current.Value.ToString() : DBNull.Value); q.Parameters.AddWithValue("$now", now.ToString()); return q.ExecuteNonQuery() == 1; }
