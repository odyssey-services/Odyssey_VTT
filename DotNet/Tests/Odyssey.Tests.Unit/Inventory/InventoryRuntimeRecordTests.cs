...
namespace Odyssey.Tests.Unit.Inventory
{
    public sealed class InventoryRuntimeRecordTests
    {
        ...
        public void InventoryRuntimeScope_AllowsOnlyMoveBehaviorBeforeLaterInventoryTasks()
        {
            ...
            Assert.That(inventoryRuntimeFiles.Where(path => Path.GetFileName(path).IndexOf("Equipment", StringComparison.OrdinalIgnoreCase) >= 0 || Path.GetFileName(path).IndexOf("Equipped", StringComparison.OrdinalIgnoreCase) >= 0).Select(Path.GetFileName), Is.EquivalentTo(new[] { "EquippedEntry.cs" }), "ODY-S05-301 may introduce only its Equipment-runtime domain vocabulary file -- no persistence, command, or RemoveBodyPart-check behavior.");
