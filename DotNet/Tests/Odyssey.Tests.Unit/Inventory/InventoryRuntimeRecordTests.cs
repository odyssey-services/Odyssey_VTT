...
namespace Odyssey.Tests.Unit.Inventory
{
    public sealed class InventoryRuntimeRecordTests
    {
        ...
        public void InventoryRuntimeScope_AllowsOnlyMoveBehaviorBeforeLaterInventoryTasks()
        {
            ...
            Assert.That(inventoryRuntimeFiles.Where(path => Path.GetFileName(path).IndexOf("Stack", StringComparison.OrdinalIgnoreCase) >= 0).Select(Path.GetFileName), Is.EquivalentTo(new[] { "InventoryStackOperationService.cs", "InventoryStackFailures.cs" }), "ODY-S05-205 may introduce only its stack split/merge service and failure factory.");
            Assert.That(inventoryRuntimeFiles.Where(path => Path.GetFileName(path).IndexOf("Equipment", StringComparison.OrdinalIgnoreCase) >= 0 || Path.GetFileName(path).IndexOf("Equipped", StringComparison.OrdinalIgnoreCase) >= 0).Select(Path.GetFileName), Is.EquivalentTo(new[] { "EquippedEntry.cs", "EquipmentService.cs" }), "ODY-S05-301 introduced the Equipment-runtime domain vocabulary file; ODY-S05-303 introduced the Equip command orchestrator -- no RemoveBodyPart-check behavior yet.");
            ...
            Assert.That(inventoryRuntimeFiles.Where(path => Path.GetFileName(path).IndexOf("Migration", StringComparison.OrdinalIgnoreCase) >= 0).Select(Path.GetFileName), Is.EquivalentTo(new[] { "ItemDefinitionMigrationRules.cs" }), "ODY-S05-401 introduced only the migration preview type/builder -- no blocking-incompatibility computation, backup call, or apply logic yet.");
